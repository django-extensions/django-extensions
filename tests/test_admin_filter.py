...
from django_extensions.admin.filter import (
    NotNullFieldListFilter, NullFieldListFilter,
)
...
class NullFieldListFilterTests(BaseFieldFilter):
    ...
    def test_should_not_filter_qs_if_all_lookup_selected(self):  # noqa
        params = {}
        filter_spec = NullFieldListFilter(
            self.field, self.request, params, Secret, Mock(), self.field_path)

        result = filter_spec.queryset(self.request, self.qs)

        self.assertQuerysetEqual(
            self.qs, map(repr, result), ordered=False)

    def test_should_return_objects_with_empty_text_if_yes_lookup_selected(self):  # noqa
        ...
        self.assertQuerysetEqual(
            expected_result, map(repr, result), ordered=False)

    def test_should_return_objects_with_not_empty_text_value_if_no_lookup_selected(self):  # noqa
    ...
    def test_choices(self):
        expected_result = [
            ...
            {'selected': False, 'query_string': '?active__isnull=1', 'display': 'Yes'},  # noqa
            {'selected': False, 'query_string': '?active__isnull=0', 'display': 'No'}  # noqa
        ]
        m_cl = Mock()
        m_cl.get_query_string.side_effect = [
            '?', '?active__isnull=1', '?active__isnull=0']
        filter_spec = NullFieldListFilter(
            self.field, self.request, {}, Secret, Mock(), self.field_path)
...
class NotNullFieldListFilterTests(BaseFieldFilter):
    ...
    def test_should_not_filter_qs_if_all_lookup_selected(self):  # noqa
        params = {}
        filter_spec = NotNullFieldListFilter(
            self.field, self.request, params, Secret, Mock(), self.field_path)
