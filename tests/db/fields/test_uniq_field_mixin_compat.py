...
from tests.testapp.models import (
    ...
    ReverseModelCompat,
    SecondDummyRelationModel,
    ThirdDummyRelationModel,
)
...
class UniqFieldMixinCompatTestCase(TestCase):
    ...
    def test__get_fields_returns_correct_fields(self):
        ...
        uniq_mixin_fields = [
            i[0] for i in UniqueFieldMixin._get_fields(PostWithUniqFieldCompat)
        ]
    ...
    def test__get_fields_returns_correct_model(self):
        post_models = [
            i[1] for i in UniqueFieldMixin._get_fields(PostWithUniqFieldCompat)
        ]
        self.assertTrue(all(model is None for model in post_models))

        inherited_post_models = [
            i[1]
            for i in UniqueFieldMixin._get_fields(InheritedFromPostWithUniqFieldCompat)
            if i[1]
        ]

        self.assertEqual(len(inherited_post_models), 6)
        self.assertTrue(
            all(model is PostWithUniqFieldCompat) for model in inherited_post_models
        )

    def test_get_queryset(self):
        mocked_get_fields = ((models.CharField, PostWithUniqFieldCompat),)
        with (
            mock.patch(
        ...
        mocked_get_fields = ((models.CharField, None),)
        ...
        with (
            ...
            self.uniq_field.get_queryset(
                InheritedFromPostWithUniqFieldCompat, models.CharField
            )
